...
This is not a position paper. The substrate described here largely built itself: across *two* autonomous, multi-hour engineering runs it produced 400+ new tests with zero regressions, on commodity inference, for a few dollars amortized. Its verification spine, a real, model-free honesty harness, caught every adversarially planted false claim in the live run's planted set, and a directed adversarial review caught a real host-level remote-code-execution path before merge. The architecture is the argument.
...
- **Long-horizon autonomy, repeated, not a one-off.** *Two* completed autonomous runs, both merged with **zero regressions**: Mega-Sprint #1 (~5 h, 9/9 sprints, **+294 tests**, 0 human interventions until the merge gate) and the Mega-Sprint #2 convergence lane (6 sprints C1-C6, **+115 tests**, core 2202→2317, each passing its own live end-to-end check and an adversarial audit *driven to convergence over 3-6 rounds*, with every corpus-touching test run against a clone and the live corpus asserted untouched). A third lane (GUI) is in flight. The pattern reproduces.
